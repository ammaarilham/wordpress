...
class ThemeMultiLanguages {
	...
	public static function pre_init() {
		if (!is_admin()) {
            ...
            add_filter('wp_get_nav_menu_items', array(__CLASS__, 'disable_server_cache_for_menu_links'), 11, 3);
    ...
    /**
     * Disable server cache for menu links
     *
     * @param $items
     *
     * @return mixed
     */
    public static function disable_server_cache_for_menu_links($items)
    {
        $randomString =  substr(str_shuffle('0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ'), 0, 5) . wp_rand(1, 5000);
        foreach ($items as $item) {
            $urlAttrs = explode('#', $item->url);
            $anchor = isset($urlAttrs[1]) ? '#' . $urlAttrs[1] : '';
            $item->url = $urlAttrs[0] . '?lang_ver=' . $randomString . $anchor;
        }
        return $items;
    }
